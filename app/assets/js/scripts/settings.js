        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Afin de vous servir du launcher vous devez avoir au moins un compte. Vous devrez vous reconnecter.<br><br>Êtes vous sûr de vouloir vous déconnecter ?',
                    'Je suis sûr',
                    'Annuler'
                )
                setOverlayHandler(() => {
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Erreur lors de la suppresion du<br> Mod ${fullName}`,
                    'Vérifiez que ce fichier n\'est pas en cours d\'utilisation et réessayez',
                    'Okay'
                )
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Erreur lors de la sélection de <br>un ou plusieurs mods',
                            err.message,
                            'Okay'
                        )
                        setOverlayHandler(null)
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/hynity/HynityLauncherJS/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Erreur lors du chargement des notes de mise à jour. '
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Téléchargez depuis GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermez le Launcher et exécutez le DMG.</span>', false, () => {
